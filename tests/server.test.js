...
test('creates and lists agents', async () => {
  ...
  assert.equal(
    list.body.some((agent) => agent.name === 'Agent A'),
    true,
  );
});
...
test('creates and starts a job', async () => {
  const create = await request(app)
    .post('/api/jobs')
    .send({ title: 'Investigate Issue #10' });
  assert.equal(create.status, 201);

  const start = await request(app)
    .post(`/api/jobs/${create.body.id}/start`)
    .send({});
...
test('creates, updates, and deletes a task', async () => {
  const create = await request(app)
    .post('/api/tasks')
    .send({ title: 'Build feature', priority: 'high', tags: 'dev,urgent' });
  assert.equal(create.status, 201);
...
test('lists tasks', async () => {
  ...
  assert.equal(Array.isArray(list.body), true);
  assert.equal(
    list.body.some((t) => t.title === 'Task A'),
    true,
  );
});
...
test('creates and advances content through pipeline', async () => {
  ...
  const advance = await request(app)
    .put(`/api/content/${create.body.id}`)
    .send({ stage: 'review' });
  assert.equal(advance.status, 200);
  assert.equal(advance.body.stage, 'review');

  const editBody = await request(app)
    .put(`/api/content/${create.body.id}`)
...
test('creates and lists events', async () => {
  ...
  assert.equal(list.status, 200);
  assert.equal(
    list.body.some((e) => e.title === 'Sprint Planning'),
    true,
  );
});
...
test('creates and updates memory with versioning', async () => {
  ...
  const update = await request(app)
    .put(`/api/memories/${create.body.id}`)
    .send({ content: 'Updated findings' });
  assert.equal(update.status, 200);
...
test('global search returns matching results', async () => {
  ...
  assert.equal(
    result.body.tasks.some((t) => t.title === 'SearchTestItem'),
    true,
  );
});
